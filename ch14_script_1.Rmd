...
logistic_fit <- logistic_mod %>%
  fit(trump ~ gender + age + educ + race + income + party + ideology,
    family = 'binomial', data = cces)
```
...
```{r}
logistic_fit %>% broom::tidy(conf.int = T) %>% select(-c(std.error:p.value))
```

The estimated average treatment effect of gender on voting for Trump is a decrease in probability of voting for Trump by 8.5% for a female voter vs a male voter, and we can be confident that the true effect is between -11.1% and -5.81%.
...
This model predicts that Dom will vote for Clinton.
...
Dom's party is less than 4, so he would be in the $0$ bucket. 
...
The OOB error tells us the proportion of the data that was incorrectly classified. The 920 represents the total number of Trump voters (0's) that were incorrecly classified as Clinton voters (1's). The class.error is the propotion of the class, Trump voters, that was incorrectly classified.
...
```{r}
tree_preds <- predict(trump_tree, new_data = cces)
log_preds <- predict(logistic_fit, new_data = cces)
forest_preds <- predict(trump_forest, new_data = cces)

cces %>%
  bind_cols(tree_preds, 
            log_preds, 
            forest_preds) %>%
  select(trump,
         tree = .pred_class,
         logist = .pred_class1,
         forest = .pred_class2) %>%
  summarize(tree.correct = mean(tree == trump),
            logist.correct = mean(logist == trump),
            forest.correct = mean(forest == trump))
```
